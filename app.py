...
# --- 1. الإعدادات ---
st.set_page_config(page_title="المستشار المالي 2026 - v64", layout="wide")

# الاعتماد على الذاكرة المؤقتة فقط (مع نظام الاستعادة)
...
# ملف الإعدادات المحلي (سيتم تصفيره عند إعادة التشغيل في الكلاود، لذا سنعتمد على التحميل)
CONFIG_FILE = "app_config_temp.json"
...
if 'df' not in st.session_state:
    st.session_state.df = pd.DataFrame(columns=['التاريخ', 'اليوم', 'النوع', 'التصنيف', 'المبلغ', 'التفاصيل'])
...
<style>
    ...
    .val-stroke-white { color: #ffffff !important; font-size: 32px !important; font-weight: 900 !important; text-shadow: 2px 2px 0 #000, -2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000; }
    .val-stroke-green { color: #22c55e !important; font-size: 32px !important; font-weight: 900 !important; text-shadow: 1.5px 1.5px 0 #000, -1px -1px 0 #000; }
    .val-stroke-red { color: #ef4444 !important; font-size: 32px !important; font-weight: 900 !important; text-shadow: 1.5px 1.5px 0 #000, -1px -1px 0 #000; }

    .warn-badge { background-color: #ef4444; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: bold; animation: blink 1s infinite; display: inline-block; margin-top: 2px; }
    @keyframes blink { 50% { opacity: 0; } }

    .svc-box { background: #1e293b; padding: 10px; border-radius: 15px; border: 2px solid #3b82f6; text-align: center; height: 140px; display: flex; flex-direction: column; justify-content: center; align-items: center; }
...
tabs = st.tabs(["📊 الرئيسية", "🛒 إضافة مصروفات (شامل)", "💰 دخل وثوابت", "🔄 مقارنات وترند", "💾 النسخ الاحتياطي (هام)"])
...
with tabs[0]:
    if not df.empty:
        ...
        sel_cycle = st.selectbox("📅 الدورة الشهرية:", cycles) if cycles else None
        ...
        if sel_cycle:
            ...
            m_inc = m_exp = m_rem = 0.0

        c1, c2, c3, c4 = st.columns(4)
        with c1: st.markdown(f"<div class='card-container' style='background:#bfdbfe;'><div class='card-icon'>💰</div><div class='text-content'><div class='card-title'>إجمالي الدخل</div><div class='val-stroke-white'>{m_inc:,.2f}</div></div></div>", unsafe_allow_html=True)
        with c2: st.markdown(f"<div class='card-container' style='background:#e9d5ff;'><div class='card-icon'>💸</div><div class='text-content'><div class='card-title'>مصروفات الشهر</div><div class='val-stroke-white'>{m_exp:,.2f}</div></div></div>", unsafe_allow_html=True)
        
        with c3:
            cls = "val-stroke-green" if m_rem >= 0 else "val-stroke-red"
            warn_html = "<div class='warn-badge'>⚠️ عجز!</div>" if m_rem < 0 else ""
            st.markdown(f"<div class='card-container'><div class='card-icon'>⚖️</div><div class='text-content'><div class='card-title'>المتبقي الشهري</div><div class='{cls}'>{m_rem:,.2f}</div>{warn_html}</div></div>", unsafe_allow_html=True)
            
        with c4:
            cls_n = "val-stroke-green" if net_savings >= 0 else "val-stroke-red"
            warn_n_html = "<div class='warn-badge'>⚠️ سالب!</div>" if net_savings < 0 else ""
            st.markdown(f"<div class='card-container'><div class='card-icon'>🏦</div><div class='text-content'><div class='card-title'>صافي المدخرات</div><div class='{cls_n}'>{net_savings:,.2f}</div>{warn_n_html}</div></div>", unsafe_allow_html=True)
        ...
        st.info("👋 مرحباً! النظام جاهز. انتقل لتبويب 'النسخ الاحتياطي' لاستعادة بياناتك، أو ابدأ بإضافة بيانات جديدة.")
...
with tabs[1]:
    ...
    with st.form("bulk_expense_form", clear_on_submit=True):
        ...
        st.divider()
        if st.form_submit_button("💾 حفظ الكل في السجل"):
            new_rows = []
            ...
            if new_rows:
                ...
                st.success(f"✅ تم إضافة {len(new_rows)} عمليات بنجاح!"); st.rerun()
...
with tabs[2]:
    ...
    with c1:
        with st.form("i", clear_on_submit=True):
            ...
            if st.form_submit_button("حفظ"): st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([{"التاريخ":pd.to_datetime(d),"اليوم":d_name,"النوع":"دخل","التصنيف":c,"المبلغ":a}])], ignore_index=True); st.rerun()
    with c2:
        with st.form("f", clear_on_submit=True):
            st.subheader("🏠 ثابت"); d=st.date_input("تاريخ"); c=st.selectbox("نوع", FIXED_CATS); a=st.number_input("مبلغ")
            if st.form_submit_button("حفظ"): st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([{"التاريخ":pd.to_datetime(d),"اليوم":d_name,"النوع":"مصروفات ثابتة","التصنيف":c,"المبلغ":a}])], ignore_index=True); st.rerun()
...
# --- Tab 5: النسخ الاحتياطي (الحل الجذري) ---
with tabs[4]:
    ...
    ⚠️ <b>تنبيه هام:</b> عند تشغيل البرنامج على الإنترنت، قد يتم مسح البيانات عند إعادة التشغيل.
    <br>✅ <b>الحل:</b> قم دائماً بتنزيل نسخة احتياطية بعد الانتهاء من التعديلات. عند العودة، قم برفع الملف لاستعادة بياناتك.
    </div>
    """, unsafe_allow_html=True)
    
    col_dl, col_up = st.columns(2)
    
    # قسم التنزيل (حفظ البيانات)
    with col_dl:
        st.write("### 1️⃣ حفظ نسخة (Download)")
        if not df.empty:
            csv = df.to_csv(index=False).encode('utf-8-sig')
            st.download_button(
                label="📥 تنزيل ملف البيانات (Excel/CSV)",
                data=csv,
                file_name=f"finance_backup_{date.today()}.csv",
                mime="text/csv",
                type="primary"
            )
        else:
            st.warning("لا توجد بيانات حالياً لتنزيلها.")

    # قسم الرفع (استعادة البيانات)
    with col_up:
        st.write("### 2️⃣ استعادة نسخة (Upload)")
        uploaded_file = st.file_uploader("ارفع ملف النسخة الاحتياطية هنا", type=['csv', 'xlsx'])
        if uploaded_file:
            try:
                ...
                # تصحيح التواريخ
                loaded_df['التاريخ'] = pd.to_datetime(loaded_df['التاريخ'], errors='coerce')
                
                if st.button("🔄 استبدال البيانات الحالية بالملف المرفوع"):
    ...
    st.divider()
    st.write("### ✏️ تعديل السجلات يدوياً")
