...
st.set_page_config(page_title="المستشار المالي 2026 - النسخة النهائية", layout="wide")
...
def load_data():
    if os.path.exists(DB_FILE):
        ...
        # تصفية لتبدأ من ميزانية شهر 1-2026 (التي تبدأ من 27-12-2025 تقريباً)
...
st.markdown(f"""
    <div style="background-color:#0f172a; padding:20px; border-radius:15px; text-align:center; color:white; border-bottom: 5px solid #3b82f6;">
        ...
        <div id="live_clock_final" style="font-size: 45px; color: #3b82f6; font-weight: bold; font-family: monospace; margin: 10px 0;">00:00:00</div>
...
components.html("""
    ...
    function update() {
        const now = new Date();
        window.parent.document.getElementById('live_clock_final').innerHTML = now.toLocaleTimeString('en-GB', { hour12: false });
    }
...
# --- Tab 1: الرئيسية (ملخص شهر 1-2026 وما بعده) ---
with tab1:
    if not df.empty:
        ...
        m4.metric("📈 صافي مدخرات 2026", f"{actual_net_surplus:,.2f}")
        ...
        if not daily_spend.empty:
            ...
            c_high.markdown(f"<div style='text-align:center; padding:15px; background:#1e293b; border-radius:15px; border:2px solid #ef4444;'><h3>🔺 أعلى صرف يومي</h3><h2>{daily_spend.max():,.2f}</h2><p>{max_day}</p></div>", unsafe_allow_html=True)
            c_low.markdown(f"<div style='text-align:center; padding:15px; background:#1e293b; border-radius:15px; border:2px solid #10b981;'><h3>🔻 أدنى صرف يومي</h3><h2>{daily_spend.min():,.2f}</h2><p>{min_day}</p></div>", unsafe_allow_html=True)

        st.divider()
        # 3. أيقونات الخدمات والهدف الادخاري
        ...
        for name, icon, col in [("ماء", "💧", col_w), ("الغاز", "🔥", col_g), ("الزيت", "🛢️", col_o)]:
            ...
            with col:
                ...
                with st.popover(f"إضافة/تعديل {name}"):
                    e_d = st.date_input(f"التاريخ", date.today(), key=f"d_{name}")
        ...
        st.divider()
        # 4. الرسم البياني وسجل العمليات المنسق
        st.write(f"### 📊 تفاصيل مصروفات شهر {sel_cycle}")
        ...
        with col_l:
            st.write("📑 **سجل عمليات الشهر المنسق (الأحدث أولاً):**")
            log_display = curr_df[['التاريخ', 'التصنيف', 'النوع', 'المبلغ', 'التفاصيل']].sort_values('التاريخ', ascending=False)
            st.dataframe(log_display, use_container_width=True, height=400)
    else:
        st.info("مرحباً بك في عام 2026! ابدأ بإدخال بياناتك الأولى.")
...
with tab4:
    if not df.empty:
        ...
        target_cat = st.selectbox("اختر العنصر لتحليله زمنياً (مثال: بنزين):", sorted(exp_all['التصنيف'].unique()))
        
        item_df = exp_all[exp_all['التصنيف'] == target_cat].copy()
        item_df['اليوم_من_الشهر'] = item_df['التاريخ'].dt.day
        
        fig_daily = px.scatter(item_df, x='اليوم_من_الشهر', y='المبلغ', color='دورة_الميزانية', size='المبلغ',
                               title=f"📈 تتبع صرف {target_cat} خلال أيام الأشهر المختلفة")
        st.plotly_chart(fig_daily, use_container_width=True)
        
        st.write("🔢 **التفاصيل الرقمية للعنصر المختار:**")
        st.dataframe(item_df[['التاريخ', 'دورة_الميزانية', 'المبلغ', 'التفاصيل']].sort_values('التاريخ', ascending=False), use_container_width=True)

# --- Tab 3: دخل وثوابت مع التاريخ ---
with tab3:
    ci, cf = st.columns(2)
    with ci:
        with st.form("inc_final"):
            st.subheader("💰 تسجيل دخل جديد")
            i_date = st.date_input("تاريخ الاستلام", date.today())
            i_cat = st.selectbox("مصدر الدخل", INCOME_CATS)
            ...
            if st.form_submit_button("حفظ الدخل"):
                ...
                st.session_state.df = pd.concat([st.session_state.df, new], ignore_index=True); save_data(st.session_state.df); st.rerun()
    with cf:
        with st.form("fix_final"):
            st.subheader("🏠 مصروف ثابت جديد")
            f_date = st.date_input("تاريخ المصروف", date.today())
            f_cat = st.selectbox("نوع المصروف الثابت", FIXED_CATS)
...
with tab5:
    ...
    with col_u:
        up = st.file_uploader("📥 استيراد ودمج ملفات (CSV/Excel)", type=['csv', 'xlsx'])
        if up:
            ...
            save_data(st.session_state.df); st.success("تم دمج البيانات بنجاح!"); st.rerun()
    with col_d:
        st.download_button("📤 تصدير بيانات 2026 كاملة", df.to_csv(index=False).encode('utf-8-sig'), "finance_2026_final.csv")

    st.divider()
    st.markdown("### 📝 محرر السجلات الموحد (تعديل وحذف)")
    edited = st.data_editor(st.session_state.df, num_rows="dynamic", use_container_width=True, key="master_ed_final")
    if st.button("💾 حفظ كافة التعديلات"):
        st.session_state.df = edited
        save_data(edited); st.success("تم التحديث!"); st.rerun()

with tab2:
    with st.form("d_final"):
